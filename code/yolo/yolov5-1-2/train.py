...
def train(hyp, opt, device, tb_writer=None):
    ...
    optimizer = torchcontrib.optim.SWA(optimizer, swa_start=10, swa_freq=5, swa_lr=0.05) # add swa
    # for _ in range(100):
    #     optSWA.zero_grad()
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                # result1 = np.array(targets)
                # np.savetxt('1.txt', result1)
                # result2 = np.array(pred)
                # np.savetxt('2.txt', pred)

                # f = open("2.txt", "w")
                # for i in range(len(pred)):
                    # f.writelines(str(pred[i][5:-1]))
        ...
        if rank in [-1, 0]:
            ...
            if save:
                ...
                if best_fitness == fi:
                    ...
                    line = "the best epoch = " + str(epoch) + '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n'
                    print(line + "\n")
                    file = open(results_file, 'a')
                    file.write(line)
                    # file.write(s + '%10.4g' * 7 % results_my + '\n')  # P, R, mAP, F1, test_losses=(GIoU, obj, cls)
                del ckpt
        # end epoch ----------------------------------------------------------------------------------------------------
    # end training
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=1000)  # 可改
    parser.add_argument('--batch-size', type=int, default=16, help='total batch size for all GPUs')  # batch_size 显卡垃圾的话，就调小点
    parser.add_argument('--img-size', nargs='+', type=int, default=[640, 640], help='[train, test] image sizes')
    parser.add_argument('--cfg', type=str, default='models/yolov5x.yaml', help='model.yaml path')  # 可改
