...
def detect(save_img=False):
    # src_txt_dir = "F:/ZYY/chess/yolov5/yolov5-1-1/inference/output/train-1-1-txt/" # 路径
    # src_txt_dir = "F:/ZYY/chess/model_ensemble/data/test/test-1-1-txt/"
    src_txt_dir = "F:/ZYY/chess/model_ensemble/data/train/train-1-1-txt/"
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if names[int(cls)] == 'empty':
                            ...
                            num = '1'
                        elif names[int(cls)] == 'black':
                            num = '2'
                        else:
                            num = '3'
                        plot_one_box(xyxy, im0, label=num, color=colors[int(cls)], line_thickness=thickness)
                        ...
                        label_bnd = num + ' ' + str(int(bnd[0])) + ' ' + str(int(bnd[1])) + ' ' + str(int(bnd[2])) + ' ' + str(int(bnd[3])) + ' %.2f'%(conf)  + '\n'
...
if __name__ == '__main__':
    ...
    # source = "F:/ZYY/chess/yolov5/yolov5-1-1/inference/images/test/"
    # out = "F:/ZYY/chess/model_ensemble/data/test/test-1-1-out/"
    source = "F:/ZYY/chess/yolov5/yolov5-1-1/inference/images/train/"
    out = "F:/ZYY/chess/model_ensemble/data/train/train-1-1-out/"
    parser.add_argument('--weights', nargs='+', type=str, default='runs/pt/1-1-CBAM.pt', help='model.pt path(s)')
    ...
    # parser.add_argument('--weights', nargs='+', type=str, default='runs/exp3/weights/best1.pt', help='model.pt path(s)') # 路径
    # parser.add_argument('--source', type=str, default='inference/images/train', help='source')  # 路径
    # parser.add_argument('--output', type=str, default='inference/output/train-1-1-out', help='output folder')  # 路径
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            ...
            # 图像存储位置
            src_img_dir = "F:/ZYY/chess/yolov5/yolov5-1-1/inference/images/train/"
            # 图像的 ground truth 的 txt 文件存放位置
            src_txt_dir = "F:/ZYY/chess/yolov5/yolov5-1-1/inference/output/forword-1-1-txt/"
            src_xml_dir = "F:/ZYY/chess/yolov5/yolov5-1-1/inference/output/forword-1-1-xml/"
